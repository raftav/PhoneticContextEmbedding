	...
	with tf.Graph().as_default():
		...
		with tf.device('/cpu:0'):
			with tf.variable_scope('model',reuse=None):
				...
				train_model = lstm_model.Model(acoustic_features,audio_labels,seq_length,config,is_training=True)
				print('done.\n')
		...
		with tf.device('/cpu:0'):
			with tf.variable_scope('model',reuse=True):
				...
				val_model = lstm_model.Model(acoustic_features_val,audio_labels_val,seq_length_val,config,is_training=None)
				print('done.')
		...
		with tf.Session(config=tf.ConfigProto(allow_soft_placement=True,
			...
			try:
				...
				while not coord.should_stop():
					...
					_ , loss  = sess.run([train_model.optimize,train_model.loss])
					...
					if (step % 100 == 0 or step==1):
						print("step[{:7d}] loss[{:2.5f}] time[{}]".\
								format(step,loss,time.time()-partial_time))
						partial_time=time.time()
					...
					if ((step % int(num_examples / config.batch_size) == 0) and (step is not 0)):
						...
						print('Training:')
						print('loss[{:2.5f}] '.format(epoch_loss))
						...
						if((epoch_counter%out_every_epoch)==0):
							...
							for i in range(config.num_examples_val):
								...
								# validation
								loss , example_accuracy = sess.run([val_model.loss,val_model.accuracy])
							...
							print('')
							print('Validation: ')
							print('loss[{:2.5f}] accuracy[{:2.5}] time[{}]'.format(val_loss,val_accuracy,time.time()-validation_time))

							
							trainingLogFile.write('{:d}\t{:.8f}\t{:.8f}\t{:.8f}\n'.format(epoch_counter,epoch_loss,val_loss,val_accuracy))
